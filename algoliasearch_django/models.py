    ...
    def reindex_all(self, batch_size=1000):
        ...
        try:
            if self.settings:
                ...
                self.__tmp_index.wait_task(
                    self.__tmp_index.set_settings(self.settings)["taskID"]
                )
            ...
            self.__client.move_index(
                self.__tmp_index.index_name, self.__index.index_name
            )
            ...
            if self.settings:
                ...
                if should_keep_synonyms:
                    response = self.__index.batch_synonyms(
                        synonyms, forward_to_replicas=True
                    )
                    self.__index.wait_task(response["taskID"])
                    logger.info(
